...
async function compressKatexFonts() {
  try {
    ...
    const existingFonts = fs.existsSync(distAstroDir)
      ? fs
          .readdirSync(distAstroDir)
          .filter((f) => f.includes("KaTeX") && f.endsWith(".woff2"))
      : [];

    for (const fontConfig of KATEX_FONTS) {
      const fontFiles = existingFonts.filter((f) =>
        f.includes(fontConfig.name),
      );
      ...
      for (const fontFile of fontFiles) {
        ...
        try {
          ...
          console.log(
            `  ${fontFile} -> copied (${(originalSize / 1024).toFixed(2)} KB)`,
          );
          processedCount++;
        } catch (error) {
    ...
    if (processedCount > 0) {
      ...
      console.log(
        `  Original size: ${(totalOriginalSize / 1024).toFixed(2)} KB`,
      );
      console.log(
